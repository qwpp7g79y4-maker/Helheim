...
impl ConsciousWorker for CleanerWorker {
    fn name(&self) -> &str { "Cleaner (System Hygiene)" }
    fn purpose(&self) -> &str { "To maintain entropy balance by organizing file chaos." }
    fn mechanism(&self) -> &str { "Scanning ~/Downloads and sorting by MIME type via 'pepai' script." }
    ...
    fn execute(&self) -> Result<String> {
        ...
        if !script_path.exists() {
            return Err(anyhow!("Worker 'cleaner' (pepai script) not found at {:?}", script_path));
        }
...
impl Swarm {
    ...
    pub fn dispatch(intent: &str) -> Result<String> {
        ...
        match intent {
            ...
            _ => Err(anyhow!("The Swarm has no conscious connection to: '{}'", intent)),
        }
    }
}
