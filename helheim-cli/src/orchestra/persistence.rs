...
impl MemoryState {
    ...
    pub async fn save(globals: &HashMap<String, String>, functions: &HashMap<String, String>) -> Result<String> {
        let state = Self {
        ...
        // Write to TMP
        fs::write(&tmp_path, json).await.context("Failed to write temp memory")?;
        
        // Rename TMP to FINAL (Atomic on POSIX)
        fs::rename(&tmp_path, &path).await.context("Failed to commit memory")?;
    ...
    pub async fn load() -> Result<Self> {
        ...
        let content = fs::read_to_string(&path).await.context("Failed to read memory")?;
        let state: MemoryState = serde_json::from_str(&content).context("Corrupt memory file")?;
