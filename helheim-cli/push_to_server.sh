...
# Configuration
REMOTE_USER="helheim_user"
REMOTE_HOST="$1" # Pass IP as argument, e.g. ./push_to_server.sh 192.168.0.50

if [ -z "$REMOTE_HOST" ]; then
    echo "❌ Gebruik: ./push_to_server.sh [SERVER_IP]"
    echo "   Voorbeeld: ./push_to_server.sh 192.168.0.200"
    exit 1
fi
