    ...
    match stmt {
        ...
        CodeTaal::Receive { var, timeout, body } => {
            ...
            let received_wrapper = if let Some(timeout_ast) = timeout {
                ...
                // No timeout provided: enforce a safety fallback timeout to prevent infinite deadlocks (30s)
                let default_duration = std::time::Duration::from_secs(30);
                match tokio::time::timeout(default_duration, rx.recv()).await {
                    Ok(Some(wrapper)) => wrapper,
                    _ => {
                        tracing::warn!("[ACTOR_DEADLOCK_PREVENTION] Actor mailbox receive liep na 30 seconden tegen de veiligheidstimeout aan. Voorkomt starvation.");
                        HelValueWrapper::Local(HelheimType::Null)
                    }
                }
            };
